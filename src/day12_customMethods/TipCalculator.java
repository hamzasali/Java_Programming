...
public class TipCalculator {
    public static void main(String[] args) {
        ...
        System.out.println("How was Service Quality? (Poor, Fair, Good, Great, Excellent)");
        String service = input.next().toLowerCase();

        input.close();

        double totalTip = (service.equals("excellent")) ? check$ * 0.25
                : (service.equals("great")) ? check$ * 0.20
                : (service.equals("good")) ? check$ * 0.15
                : (service.equals("fair")) ? check$ * 0.10
                : check$ * 0.05;

        double totalBill = check$ + totalTip;
        ...
        if (splitOrNo.equals("yes")) {
            System.out.println("Total per person: " + (totalBill / numOfPeople));
